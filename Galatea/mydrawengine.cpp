...
ErrorType MyDrawEngine::Start(HWND hwnd, bool bFullScreen)
{
	...
	instance->AddFont(L"Courier New", 24, false, false);
  instance->AddFont(L"Courier New", 34, true, false); // Genral Game Font
  instance->AddFont(L"Courier New", 32, true, true);  // Upgrades Font
  instance->AddFont(L"Courier New", 62, true, false); // Title Font
  instance->AddFont(L"Courier New", 42, false, false); // Scores Font
  instance->AddFont(L"Courier New", 42, true, true); // Your Score Font
  instance->AddFont(L"Courier New", 20, false, false); // Credits Font
...
{
	//Clear
	HRESULT err = m_lpD3DDevice->Clear(0, NULL, D3DCLEAR_TARGET, D3DCOLOR_ARGB(255, 15,15,50),1.0f,0);
...
// Nick
ErrorType MyDrawEngine::WriteText(Vector2D position, std::string text, int colour, FontIndex fontIndex)
{
  size_t size = text.length() + 1, outsize;

  wchar_t *w = new wchar_t[size];
  const char *c = text.c_str();
  ErrorType e;

  mbstowcs_s(&outsize, w, size, c, size);

  e = WriteText(position, w, colour, fontIndex);
